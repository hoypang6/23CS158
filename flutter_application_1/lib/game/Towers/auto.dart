...
class Auto extends SpriteComponent with CollisionCallbacks, HasGameRef<GameSelf>{ 
  ...
  void onLoad() async{
    await super.onLoad();
    sprite = await gameRef.loadSprite('auto.png'); //switch to unload, need to check
    size = Vector2(40, 70);
    position = dropPosition; //sideways, height
    anchor = Anchor.center;
    
    add(
      CircleHitbox(radius: 140,anchor: Anchor.center, position: Vector2(20.0,35.0))
    );
  ...
  callingShots(){
    if(fire == true){
      gameRef.autoBulletFactory(position, target!);
    }
