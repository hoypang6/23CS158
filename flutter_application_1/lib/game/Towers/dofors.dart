...
class Dofors extends SpriteComponent with CollisionCallbacks, HasGameRef<GameSelf>{ 
  ...
  void onLoad() async{
    await super.onLoad();
    sprite = await gameRef.loadSprite('boforsloaded.png'); //switch to unload, need to check
    size = Vector2(11, 70);
    position = dropPosition; //sideways, height
    anchor = Anchor.center;
    
    add(
      CircleHitbox(radius: 100,anchor: Anchor.center,position: Vector2(5.5,35.0))
    );
  ...
  callingShots(){
    if(fire == true){
      gameRef.doforsBulletFactory(position, target!);
    }
