...
class Qffh extends SpriteComponent with CollisionCallbacks, HasGameRef<GameSelf>{ 
  ...
  void onLoad() async{
    await super.onLoad();
    sprite = await gameRef.loadSprite('qffh.png'); //switch to unload, need to check
    size = Vector2(38, 70);
    position = dropPosition; //sideways, height
    anchor = Anchor.center;
    
    add(
      CircleHitbox(radius: 130,anchor: Anchor.center,position: Vector2(19.0,35.0))
    );
  ...
  callingShots(){
    if(fire == true){
      gameRef.qffhBulletFactory(position, target!);
    }
