...
class GameSelf extends FlameGame with TapDetector,PanDetector{ //now pushes only Level 1
  ...
  void noBuild(Vector2 dropPos){
    ...
    print(dropPos);
    if(dropPos.x > 90 && dropPos.x <= 235 && dropPos.y >0 && dropPos.y <390){
      print("w1 violation");
      notBuilding = true;
    }
    if(dropPos.x > 90 && dropPos.x <= 340 && dropPos.y >264 && dropPos.y <375){
      print("w2 violation");
      notBuilding = true;
    }
    if(dropPos.x > 270 && dropPos.x <345 && dropPos.y > 30 && dropPos.y <300){
      print("w3 violation");
      notBuilding = true;
    }
    if(dropPos.x > 270 && dropPos.x < 625 && dropPos.y > 30 && dropPos.y < 100){
      print("w4 violation");
      notBuilding=true;
    }
    if(dropPos.x > 555 && dropPos.x < 625 && dropPos.y > 30 && dropPos.y < 225){
      print("w5 violation");
      notBuilding=true;
    }
    if(dropPos.x > 370 && dropPos.x < 625 && dropPos.y > 180 && dropPos.y < 225){
      print("w6 violation");
      notBuilding=true;
    }
    if(dropPos.x > 370 && dropPos.x < 435 && dropPos.y > 250 && dropPos.y < 330){
      print("w7 violation");
      notBuilding=true;
    }
    if(dropPos.x > 370 && dropPos.x < 780 && dropPos.y > 250 && dropPos.y < 320){
      print("w8 violation");
      notBuilding=true;
    }
    if(dropPos.x<130){
  ...
  void onWin(){
    ...
    stageWin = true;
    saves.setBool('level2BCleared',stageWin);
